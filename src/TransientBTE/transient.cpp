...
Transient::Transient(BTEMesh *mesh, BTEBoundaryCondition *bcs, BTEBand *bands,
                     BTEAngle *angles, int num_proc, int world_rank,
    ...
    for (int i = 0; i < num_proc; ++i) {
        if (i < numCell % num_proc) {
            ...
            startCellList[i] = (numCell / num_proc + 1) * (numCell % num_proc) +
                               (numCell / num_proc) * (i - numCell % num_proc);
            // if(world_rank==0)
            // cout<<startCellList[i]<<endl;
        }
    }
    ...
    for (int i = 0; i < numCell; ++i) {
        for (int j = 0; j < elementFaceSize[i]; ++j) {
            elementFaceVertexesSize[j + i * 6] =
                    mesh->Elements[i].faces[j].vertexes.size();
        }
    }
    ...
    for (int i = 0; i < numCell; ++i) {
        for (int j = 0; j < elementFaceSize[i]; ++j) {
            for (int k = 0; k < elementFaceVertexesSize[j + i * 6]; ++k) {
                elementFaceVertexes[k + j * 4 + i * 6 * 4] =
                        mesh->Elements[i].faces[j].vertexes[k];
            }
        }
    }
    ...
    for (int i = 0; i < numCell; ++i) {
        for (int j = 0; j < elementVertexesSize[i]; ++j) {
            ...
            for (int k = 0; k < mesh->Nodes[vertex].cells.size(); ++k) {
                ...
                if (!ishave(cell, elementNeighborList[i]) &&
                    mesh->Elements[cell].matter == mesh->Elements[i].matter &&
                    cell != i) {
    ...
    for (int imatter = 0; imatter < numofMatter; ++imatter) {
        for (int i = 0; i < numBand; ++i) {
            for (int j = 0; j < numDirection; ++j) {
                groupVelocityX[imatter][i][j] =
                        bands->bands[i].group_velocity[imatter] * angles->direction[j].x;
                groupVelocityY[imatter][i][j] =
                        bands->bands[i].group_velocity[imatter] * angles->direction[j].y;
                groupVelocityZ[imatter][i][j] =
                        bands->bands[i].group_velocity[imatter] * angles->direction[j].z;
                heatCapacity[imatter][i][j] = bands->bands[i].heat_capacity[imatter];
                relaxationTime[imatter][i][j] =
                        bands->bands[i].relaxation_time[imatter];
    ...
    for (int iband_local = 0; iband_local < numBandLocal; iband_local++) {
        for (int inf_local = 0; inf_local < numDirectionLocal; inf_local++) {
            ...
            for (int kk = 0; kk < numCell; kk++) {
                energyDensity[iband_local][inf_local][kk] =
                        temperature[kk] * heatCapacity[matter[kk]][iband][inf];
            }
        }
    }
}
                ...
                if (dotproduct >= 0) {
                    ...
                    double e =
                            (energyDensity[iband_local][inf_local][ie] +
                             (ax * gradientX[ie] + ay * gradientY[ie] + az * gradientZ[ie]) *
                             limit[ie]);
                        ...
                        if (heatRatio[matter[ie]][iband][inf] != 0) {
                            ...
                            if (dotproduct < 0) {
                                if (boundaryType[ib] == 1) {
                                    ...
                                    eboundLocal[iband_local * numBound * 2 + ib * 2 + icell] = e;
                                    // cout<<eboundLocal[iband_local * numBound * 2 + ib * 2
                                    // +icell]<<endl;
                                    ...
                                    eboundLocal[iband_local * numBound * 2 + ib * 2 + icell] =
                                            ebound[iband * numDirection * numBound * 2 +
                                                   nf * numBound * 2 + ib * 2 + icell];
                                    ...
                                    for (int nft = 0; nft < numDirection; ++nft) {
                                        double groupvelocity1 =
                                                sqrt(pow(groupVelocityX[matter[ie]][iband][nft], 2) +
                                                     pow(groupVelocityY[matter[ie]][iband][nft], 2) +
                                                     pow(groupVelocityZ[matter[ie]][iband][nft], 2));
                                        double groupvelocity2 =
                                                sqrt(pow(groupVelocityX[matter[ie1]][iband][nft], 2) +
                                                     pow(groupVelocityY[matter[ie1]][iband][nft], 2) +
                                                     pow(groupVelocityZ[matter[ie1]][iband][nft], 2));

                                        double dotproduct1 =
                                                (directionX[nft] * elementFaceNormX[jface + ie * 6] +
                                                 directionY[nft] * elementFaceNormY[jface + ie * 6] +
                                                 directionZ[nft] * elementFaceNormZ[jface + ie * 6]);
                                        // double
                                        // dotproduct1=(groupVelocityX[matter[ie]][iband][nft] *
                                        // elementFaceNormX[jface + ie * 6] +
                                        // groupVelocityY[matter[ie]][iband][nft]
                                        //* elementFaceNormY[jface + ie * 6] +
                                        //groupVelocityZ[matter[ie]][iband][nft] *
                                    ...
                                    double groupvelocity1 =
                                            sqrt(pow(groupVelocityX[matter[ie]][iband][inf], 2) +
                                                 pow(groupVelocityY[matter[ie]][iband][inf], 2) +
                                                 pow(groupVelocityZ[matter[ie]][iband][inf], 2));

                                    double e =
                                            einsum1 / temp1 * (1 - boundaryTrans[ib][iband][icell]) +
                                            einsum2 / temp2 * (1 - boundaryTrans[ib][iband][icell]);
                                    // mesh->Boundaries[i].macro_Temp[j].Temp+=bands->bands[iband].lattice_ratio[mesh->Elements[ie].matter]
                                    // * e /
                                    // bands->bands[iband].heat_capacity[mesh->Elements[ie].matter]
                                    // * angles->weight[inf] ;
                                    ...
                                    double e = ebound[iband * numDirection * numBound * 2 +
                                                      inf * numBound * 2 +
...
void Transient::_get_heat_flux(int iband_local, int inf_local) const {
    ...
    for (int ie = 0; ie < numCell; ++ie) {
        heatFluxXLocal[ie] += groupVelocityX[matter[ie]][iband][inf] *
                              modeWeight[matter[ie]][iband][inf] *
                              energyDensity[iband_local][inf_local][ie];
        heatFluxYLocal[ie] += groupVelocityY[matter[ie]][iband][inf] *
                              modeWeight[matter[ie]][iband][inf] *
                              energyDensity[iband_local][inf_local][ie];
        heatFluxZLocal[ie] += groupVelocityZ[matter[ie]][iband][inf] *
                              modeWeight[matter[ie]][iband][inf] *
                              energyDensity[iband_local][inf_local][ie];
    }
#else
...
bool Transient::_get_magin_check_error(int nt, double error_temp_limit,
    ...
    double heat_max = 0;
    for (int i = 0; i < numCell; ++i) {
        double heat = sqrt(pow(heatFluxXGlobal[i], 2) + pow(heatFluxYGlobal[i], 2) +
                           pow(heatFluxZGlobal[i], 2));
        double heat1 = sqrt(pow(heatFluxXOld[i], 2) + pow(heatFluxYOld[i], 2) +
                            pow(heatFluxZOld[i], 2));
        if (heat_max < heat) {
            heat_max = heat;
        }
    ...
    if (worldRank == 0) {
        ...
        // << endl;
        cout << "Iteration #" << nt
             << "\t Margin per band per cell: Temperature:" << error
             << " Heat Flux:" << error_heat << endl;
    }
    if ((error > errorOld && error_heat > errorOld_heat) || isnan(error) ||
        isnan(error_heat) || isinf(error_heat) || isinf(error)) {
        errorIncreaseTime++;
    } else {
        errorIncreaseTime = 0;
    }
...
void Transient::_print_out() const {
    ...
    for (int i = 0; i < numCell; ++i) {
        output << elementCenterX[i] << " " << elementCenterY[i] << " "
               << elementCenterZ[i] << " " << temperature[i] << endl;
    }
    output.close();
    ofstream output1("Tempcell1.dat");
    for (int i = 0; i < numCell; ++i) {
        output1 << elementCenterX[i] << " " << elementCenterY[i] << " "
                << elementCenterZ[i] << " " << temperature1[i] << endl;
    }
    output1.close();
    ofstream output2("HeatSource.dat");
    for (int i = 0; i < numCell; ++i) {
        output2 << elementCenterX[i] << " " << elementCenterY[i] << " "
                << elementCenterZ[i] << " " << elementHeatSource[i] << endl;
    }
    output2.close();
    ofstream outputheat("HeatFlux.dat");
    for (int i = 0; i < numCell; ++i) {
        outputheat << setw(15) << elementCenterX[i] << setw(15) << elementCenterY[i]
                   << setw(15) << elementCenterZ[i] << setw(15)
                   << heatFluxXGlobal[i] << setw(15) << heatFluxYGlobal[i]
                   << setw(15) << heatFluxZGlobal[i] << endl;
    }
}
    ...
    if (dimension == 2) {
        for (int i = 0; i < numCell; ++i) {
            ...
            for (int j = 0; j < elementNeighborList[i].size(); ++j) {
                ...
                J1[j][0] =
                        (1.0 / L_x *
                         (elementCenterX[elementNeighborList[i][j]] - elementCenterX[i]));
                J1[j][1] =
                        (1.0 / L_x *
                         (elementCenterY[elementNeighborList[i][j]] - elementCenterY[i]));
                // d1[nump] = 1.0/L_x *
                // (energyDensity[iband_local][inf_local][elementFaceNeighobr[j+i*6]] -
        ...
        for (int i = 0; i < numCell; ++i) {
            ...
            for (int j = 0; j < elementNeighborList[i].size(); ++j) {
                ...
                J1[j][2] =
                        (1.0 / L_x *
                         (elementCenterZ[elementNeighborList[i][j]] - elementCenterZ[i]));
...
void Transient::_get_gradient_larger(int Use_limiter, int iband_local,
                                     int inf_local) const {
    int inf = ((inf_local) * numProc + worldRank) % numDirection;
    int iband = iband_local * (ceil(double(numProc) / double(numDirection))) +
                worldRank / numDirection;
    ...
    if (dimension == 1) {
        ...
        for (int ie = 0; ie < numCell; ++ie) {
            if (elementFaceBound[ie * 6] == -1 &&
                ...
                double s1 = (energyDensity[iband_local][inf_local][ie] -
                             energyDensity[iband_local][inf_local][ie - 1]) /
                            (elementVolume[ie] / 2 + elementVolume[ie - 1] / 2);
                double s2 = (energyDensity[iband_local][inf_local][ie + 1] -
                             energyDensity[iband_local][inf_local][ie]) /
                            (elementVolume[ie] / 2 + elementVolume[ie + 1] / 2);
                if ((abs(s1) + abs(s2)) != 0)
                    gradientX[ie] =
                ...
                double s1 = (energyDensity[iband_local][inf_local][ie + 1] -
                             energyDensity[iband_local][inf_local][ie]) /
                            (elementVolume[ie] / 2 + elementVolume[ie + 1] / 2);
                double s2 = s1;
                if ((abs(s1) + abs(s2)) != 0)
        ...
        for (int i = 0; i < numCell; ++i) {
            ...
            for (int j = 0; j < elementFaceSize[i]; ++j) {
                if (elementFaceBound[i * 6 + j] == -1) {
                    if (energyDensity[iband_local][inf_local]
                        [elementFaceNeighobr[j + i * 6]] > max) {
                        max = energyDensity[iband_local][inf_local]
                        [elementFaceNeighobr[j + i * 6]];
                    }
                    if (energyDensity[iband_local][inf_local]
                        [elementFaceNeighobr[j + i * 6]] < min) {
                        min = energyDensity[iband_local][inf_local]
                        [elementFaceNeighobr[j + i * 6]];
                    }
                } else if (boundaryType[elementFaceBound[i * 6 + j]] != 3 &&
                           groupVelocityX[matter[i]][iband][inf] *
                           elementFaceNormX[j + i * 6] +
                           groupVelocityY[matter[i]][iband][inf] *
                           elementFaceNormY[j + i * 6] +
                           groupVelocityZ[matter[i]][iband][inf] *
                           elementFaceNormZ[j + i * 6] <
                           0) {
                    if (ebound[iband * numDirection * numBound * 2 + inf * numBound * 2 +
                               elementFaceBound[i * 6 + j] * 2 + 0] > max) {
                        max = ebound[iband * numDirection * numBound * 2 +
                                     inf * numBound * 2 + elementFaceBound[i * 6 + j] * 2 +
                                     0];
                    }
                    if (ebound[iband * numDirection * numBound * 2 + inf * numBound * 2 +
                               elementFaceBound[i * 6 + j] * 2 + 0] < min) {
                        min = ebound[iband * numDirection * numBound * 2 +
                                     inf * numBound * 2 + elementFaceBound[i * 6 + j] * 2 +
                                     0];
                    }
                }
            ...
            for (int j = 0; j < elementFaceSize[i]; ++j) {
                ...
                if ((gradientX[i] * ax + gradientY[i] * ay + gradientZ[i] * az) *
                    limit[i] +
                    energyDensity[iband_local][inf_local][i] <
                    min) {
                    double y =
                            (min - energyDensity[iband_local][inf_local][i]) /
                ...
                if ((gradientX[i] * ax + gradientY[i] * ay + gradientZ[i] * az) *
                    ...
                    max) {
                    double y =
                            (max - energyDensity[iband_local][inf_local][i]) /
                            (gradientX[i] * ax + gradientY[i] * ay + gradientZ[i] * az);
                    limit[i] = (pow(y, 2) + 2 * y) / (pow(y, 2) + y + 2);
                }
            }
...
void Transient::_get_explicit_Re(int itime, int spatial_order, int Use_limiter,
...
#ifndef USE_GPU  // max_y is not used in following code
    // maxy要加在前面
    double max_y = 0;
    for (int ie = 0; ie < numCell; ie++) {
    ...
    for (int ie = 0; ie < numCell; ++ie) {
        for (int jface = 0; jface < elementFaceSize[ie]; ++jface) {
            ...
            if (dotproduct >= 0) {
                ...
                if (elementFaceBound[jface + ie * 6] == -1) {
                    ...
                    double ax =
                            elementFaceCenterX[jface + ie * 6] - elementCenterX[neiindex];
                    double ay =
                            elementFaceCenterY[jface + ie * 6] - elementCenterY[neiindex];
                    double az =
                            elementFaceCenterZ[jface + ie * 6] - elementCenterZ[neiindex];
                    double e = (energyDensity[iband_local][inf_local][neiindex] +
                                (ax * gradientX[neiindex] + ay * gradientY[neiindex] +
                                 az * gradientZ[neiindex]) *
                                limit[neiindex]);
                    Re[ie] += temp * e;
                    // if(ie==0)
                    // cout<<ie<<"dot<0 "<<temp<<"  "<<e<<endl;
        ...
        if (use_TDTR == 1) {
            ...
            double h =
                    heatindex * heatRatio[matter[ie]][iband][inf] * elementHeatSource[ie];
            Re[ie] -= h;
        }
    }
#else
    ...
    MIGRATE_TO_DEVICE_1D(d_heatRatio, h_heatRatio, numCell, double);
    MIGRATE_TO_DEVICE_1D(d_heatCapacity, h_heatCapacity, numCell, double);
    ...
    calcGetExplicitRe<<<1, numCell>>>(use_TDTR, deltaTime, d_elementFaceSize, repetition_frequency,
                                      ...
                                      d_elementCenterX, d_elementCenterY, d_elementCenterZ, d_energyDensity,
                                      d_gradientX, d_gradientY, d_gradientZ,
        ...
        for (int icell = 0; icell < 2; ++icell) {
            ...
            if (ie >= 0) {
                double dotproduct = (groupVelocityX[matter[ie]][iband][inf] *
                                     ...
                                     groupVelocityZ[matter[ie]][iband][inf] *
                                     elementFaceNormZ[jface + ie * 6]);
                // cout<<"  ie "<<ie<<"  dot "<<dotproduct<<"
                // groupVelocityX[matter[ie]][iband][inf]"<<groupVelocityX[matter[ie]][iband][inf]<<"iband
                // local"<<iband<<"  "<<iband_local<<"inf local"<<inf<<" "<<inf_local<<"
                // elementFaceNormX[jface + ie * 6] "<<elementFaceNormX[jface + ie *
                // 6]<<endl;
                if (dotproduct < 0) {
                    double temp =
                            elementFaceArea[jface + ie * 6] / elementVolume[ie] * dotproduct;
                    Re[ie] += temp * ebound[iband * numDirection * numBound * 2 +
                                            inf * numBound * 2 + ib * 2 + icell];
                    //   cout<<"eebound "<<ebound[iband * numDirection * numBound * 2 +
                    //   inf * numBound * 2 + ib * 2 + icell]<<endl;
                    // if(ie==0) cout<<ie<<" bound "<<temp<<"   "<<ebound[iband *
                    // numDirection * numBound * 2 + inf * numBound * 2 + ib * 2 +
                    // icell]<<endl;
                }
            }
        }
