...
int main(int argc, char **argv) {
  ...
  if (world_rank == 0) {
    cout << num_proc << " process(es),   " << device_count << " device(s)"
         << endl;
  }
  MPI_Barrier(MPI_COMM_WORLD);
  cout << "Bind solver rank " << world_rank << " to device " << device_id << "."
       << endl;
#endif

  MPI_Barrier(MPI_COMM_WORLD);
  if (world_rank == 0) {
    // cout<<"If you use GiftBTE for your research, Please cite our paper:
    // GiftBTE: An efficient deterministic solver for non-gray phonon Boltzmann
    // transport equation"<<endl;
    cout << "******************************** INTIALIZATIONS "
            "********************************"
         << endl;
  }
  ...
  if (!fin_const1.is_open()) {
    ...
    while (getline(fin_const1, str)) {
      ...
      if (str.find("IterativeScheme") >= 0 &&
          str.find("IterativeScheme") < str.length() &&
          str.find(ant) == str.npos) {
        fin_const1 >> Method;
      }
      if (str.find("MatrixSolver") >= 0 &&
          str.find("MatrixSolver") < str.length() &&
          str.find(ant) == str.npos) {
      ...
      if (str.find("MaxNumIter") >= 0 &&
          str.find("MaxNumIter") < str.length() && str.find(ant) == str.npos) {
        fin_const1 >> Num_Max_Iter;
      }
      if (str.find("ResidualTemp") >= 0 &&
          str.find("ResidualTemp") < str.length() &&
          str.find(ant) == str.npos) {
        fin_const1 >> error_temp_limit;
      }
      if (str.find("ResidualFlux") >= 0 &&
          str.find("ResidualFlux") < str.length() &&
          str.find(ant) == str.npos) {
        fin_const1 >> error_flux_limit;
      }
      if (str.find("Multiscale") >= 0 &&
          str.find("Multiscale") < str.length() && str.find(ant) == str.npos) {
        fin_const1 >> Name_multiscale_File;
      }
      if (str.find("TimeStep") >= 0 && str.find("TimeStep") < str.length() &&
          str.find(ant) == str.npos) {
        fin_const1 >> DeltaT;
      }
      ...
      if (str.find("InitialTemperatureFile") >= 0 &&
          str.find("InitialTemperatureFile") < str.length() &&
          str.find(ant) == str.npos) {
        fin_const1 >> Name_InitialTemp_File; // jiaxuan
      }
      // TDTR
      if (str.find("IsTDTR") >= 0 && str.find("IsTDTR") < str.length() &&
          str.find(ant) == str.npos) {
        fin_const1 >> use_TDTR;
        if (world_rank == 0)
      ...
      if (str.find("RepetitionFrequency") >= 0 &&
          str.find("RepetitionFrequency") < str.length() &&
          str.find(ant) == str.npos) {
        fin_const1 >> repetition_frequency;
        if (world_rank == 0)
          cout << "repetition_frequency: " << repetition_frequency << endl;
      }
      if (str.find("ModulationFrequency") >= 0 &&
          str.find("ModulationFrequency") < str.length() &&
          str.find(ant) == str.npos) {
        fin_const1 >> modulation_frequency;
        if (world_rank == 0)
          cout << "modulation_frequency: " << modulation_frequency << endl;
      }
      if (str.find("RadiusProbe") >= 0 &&
          str.find("RadiusProbe") < str.length() && str.find(ant) == str.npos) {
        fin_const1 >> xy_r;
        if (world_rank == 0)
  ...
  if (State == "Transient" && (TotalT == -1 || DeltaT == -1)) {
    cout << "Error: Please provide DeltaTime and TotalTime for transient solver"
         << endl;
    exit(0);
  }
  ...
  if (!fin_const2.is_open()) {
    ...
    while (getline(fin_const2, str)) {
      if (str.find("GeometryDimension") >= 0 &&
          str.find("GeometryDimension") < str.length() &&
          str.find(ant) == str.npos) {
        fin_const2 >> Dimension_Geometry;
      }
      if (str.find("ScaleX") >= 0 && str.find("ScaleX") < str.length() &&
          str.find(ant) == str.npos) {
        fin_const2 >> L_x;
      }
      if (str.find("ScaleY") >= 0 && str.find("ScaleY") < str.length() &&
          str.find(ant) == str.npos) {
        fin_const2 >> L_y;
      }
      if (str.find("ScaleZ") >= 0 && str.find("ScaleZ") < str.length() &&
          str.find(ant) == str.npos) {
        fin_const2 >> L_z;
      }
      if (str.find("UniformHeat") >= 0 &&
          str.find("UniformHeat") < str.length() && str.find(ant) == str.npos) {
  ...
  if (Dimension_Geometry == -1) {
    ...
    cout << "Error: Please provide Lx and Ly for two dimensional problem"
         << endl;
    exit(0);
  } else if (Dimension_Geometry == 3 && (L_x == 0 || L_y == 0 || L_z == 0)) {
    cout << "Error: Please provide Lx, Ly and Lz for two dimensional problem"
         << endl;
    exit(0);
  }
  ...
  while (getline(inputangle, str)) {
    if (str.find("MaterialDimension") >= 0 &&
        str.find("MaterialDimension") < str.length() &&
        str.find(ant) == str.npos) {
      inputangle >> Dimension_Material;
    }
    if (str.find("Angle_method") >= 0 &&
        str.find("Angle_method") < str.length() && str.find(ant) == str.npos) {
  ...
  inputangle.close();
  if (world_rank == 0) {
    // cout << "mesh: " << Name_Mesh_File  << " bcfile " << Name_Bc_File << "
    // L_x " << L_x << " ntheta " << Num_Theta << " nphi " << Num_Phi << endl;
    cout << "Order=" << Order << "  IterativeScheme=" << Method
         << "  MatrixSolver=" << Matrix_solver << endl;
    cout << "GeometryDimension=" << Dimension_Geometry << "  ScaleX=" << L_x
         << "  ScaleY=" << L_y << "  ScaleZ=" << L_z << endl;
    cout << "meshtype=" << Name_Mesh_Type << "  meshfile=" << Name_Mesh_File
         << "  bcfile=" << Name_Bc_File << endl;
  ...
  // mesh = new BTEMesh(Dimension_Geometry, geofile, L_x, L_y, L_z, bands, bcs,
  // "default"); mesh->setMeshParams(bands); mesh->setMeshParams1(bcs);
  // mesh->BTEMesh_heatin(heatfile, Uniform_heat);
  ifstream geofile1(Name_Mesh_File);
  DistributeMesh *distributeMesh;
  distributeMesh = new DistributeMesh(Dimension_Geometry, geofile1, L_x, L_y,
                                      L_z, Name_Mesh_Type, bands, bcs, heatfile,
                                      Uniform_heat, Name_Heat_Type,
                                      initialtempfile, Tref); // yufei jiaxuan
  geofile.close();
  heatfile.close();
  ...
  if (State != "Transient") {
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    distributeMesh->_build_BTEMesh(Dimension_Geometry, L_x, L_y, L_z, bands,
                                   bcs, heatfile, Uniform_heat,
                                   Name_multiscale_File);
    MPI_Barrier(MPI_COMM_WORLD);
    solutionAll._BTE_Solver(distributeMesh, bcs, bands, angles, num_proc,
                            world_rank, Use_Backup, Num_Max_Iter, Order, Method,
                            Matrix_solver, error_temp_limit, error_flux_limit);
    if (world_rank == 0) {
      solutionAll._print_out(distributeMesh);
    }
  } else {
    solutionAll._Transient_BTE_Solver(
        distributeMesh, bcs, bands, angles, num_proc, world_rank, Use_Backup,
        Order, error_temp_limit, error_flux_limit, DeltaT, TotalT, use_TDTR,
        pulse_time, repetition_frequency, modulation_frequency, xy_r);
  }
