...
SolutionAll::SolutionAll(DistributeMesh *mesh, BTEBoundaryCondition *bcs,
                         BTEBand *bands, BTEAngle *angles, int num_proc,
                         int world_rank) {
  ...
  for (int j = 0; j < numBandLocal; ++j) {
    ...
    for (int k = 0; k < numDirectionLocal; ++k) {
      solution.energyDensity[j][k] =
          new double[mesh->volumeElementsFourier.size()];
    }
  }
...
void SolutionAll::_set_initial(DistributeMesh *mesh, BTEBand *bands,
                               BTEAngle *angles) {

  for (int j = 0; j < solution.numCell; ++j) {
    // solution.Temperature[j]=0;
    solution.Temperature[j] =
        mesh->FourierMeshes.Elements[j]
            .initial_temperature; // jiaxuan: for both steady and transient
                                  // state
...
void SolutionAll::_print_out(DistributeMesh *mesh) {
  ...
  for (int i = 0; i < mesh->volumeElementsFourier.size(); ++i) {
    output << mesh->FourierMeshes.Elements[i].center.x << " "
           << mesh->FourierMeshes.Elements[i].center.y << " "
           << mesh->FourierMeshes.Elements[i].center.z << " "
           << solution.Temperature[i] << endl;
  }
  output.close();
  ...
  for (int i = 0; i < mesh->volumeElementsFourier.size(); ++i) {
    outputheat << setw(15) << mesh->FourierMeshes.Elements[i].center.x
               << setw(15) << mesh->FourierMeshes.Elements[i].center.y
               << setw(15) << mesh->FourierMeshes.Elements[i].center.z
               << setw(15) << solution.heatFluxX[i] << setw(15)
               << solution.heatFluxY[i] << setw(15) << solution.heatFluxZ[i]
               << endl;
  }
  ...
  // cout<<"======================================heat flux
  // ave====================="<<endl; ofstream outave("aveHeat");
  ...
  outputVTK << "POINTS " << mesh->FourierMeshes.Nodes.size() << " float"
            << endl;
  for (int i = 0; i < mesh->FourierMeshes.Nodes.size(); ++i) {
    outputVTK << mesh->FourierMeshes.Nodes[i].x / mesh->FourierMeshes.L_x << " "
              << mesh->FourierMeshes.Nodes[i].y / mesh->FourierMeshes.L_y << " "
              << mesh->FourierMeshes.Nodes[i].z / mesh->FourierMeshes.L_z
              << endl;
  }
  int totalnum = 0;
  ...
  outputVTK << endl;
  outputVTK << "CELLS " << mesh->FourierMeshes.Elements.size() << " "
            << totalnum << endl;
  for (int i = 0; i < mesh->FourierMeshes.Elements.size(); ++i) {
  ...
  for (int i = 0; i < mesh->FourierMeshes.Elements.size(); ++i) {
    if (mesh->FourierMeshes.Dimension == 2 &&
        mesh->FourierMeshes.Elements[i].vertexes.size() == 4) {
      outputVTK << 8 << endl;
    } else if (mesh->FourierMeshes.Dimension == 2 &&
               mesh->FourierMeshes.Elements[i].vertexes.size() == 3) {
      outputVTK << 5 << endl;
    } else if (mesh->FourierMeshes.Dimension == 3 &&
               mesh->FourierMeshes.Elements[i].vertexes.size() == 4) {
      outputVTK << 10 << endl;
    } else if (mesh->FourierMeshes.Dimension == 3 &&
               mesh->FourierMeshes.Elements[i].vertexes.size() == 8) {
      outputVTK << 11 << endl;
    } else if (mesh->FourierMeshes.Dimension == 3 &&
               mesh->FourierMeshes.Elements[i].vertexes.size() == 6) {
      outputVTK << 13 << endl;
    }
  }
  outputVTK << endl;
  outputVTK << "CELL_DATA"
            << " " << mesh->FourierMeshes.Elements.size() << endl;
  outputVTK << "SCALARS"
            << " Temperature"
            << " float 1" << endl;
  ...
  // outputVTK<<"LOOKUP_TABLE default"<<endl;
  for (int i = 0; i < mesh->FourierMeshes.Elements.size(); ++i) {
    outputVTK << solution.heatFluxX[i] << " " << solution.heatFluxY[i] << " "
              << solution.heatFluxZ[i] << endl;
  }
...
void SolutionAll::_Fourier_Solver(DistributeMesh *mesh,
                                  BTEBoundaryCondition *bcs, BTEBand *bands,
                                  int num_proc, int world_rank) const {
...
void SolutionAll::_BTE_Solver(DistributeMesh *mesh, BTEBoundaryCondition *bcs,
                              BTEBand *bands, BTEAngle *angles, int num_proc,
                              int world_rank, int Use_Backup, int Num_Max_Iter,
                              string Order, string Method, string Matrix_solver,
                              double error_temp_limit,
                              double error_flux_limit) const {
  MPI_Barrier(MPI_COMM_WORLD);
  StaticBTESynthetic solver(&mesh->BTEMeshes, bcs, bands, angles, num_proc,
                            world_rank);
  ...
  for (int ib = 0; ib < mesh->BTEMeshes.Boundaries.size(); ++ib) {
    ...
    if (mesh->FourierMeshes.Elements[mesh->elementProjection[ie]]
            .faces[jface]
            .bound == -1) {
      double distance1 = get_distance(
          mesh->FourierMeshes.Elements[mesh->elementProjection[ie]].center,
          mesh->FourierMeshes.Elements[mesh->elementProjection[ie]]
              .faces[jface]
              .center);
      double distance2 =
          get_distance(mesh->FourierMeshes
                           .Elements[mesh->FourierMeshes
                                         .Elements[mesh->elementProjection[ie]]
                                         .faces[jface]
                                         .neighbor]
                           .center,
                       mesh->FourierMeshes.Elements[mesh->elementProjection[ie]]
                           .faces[jface]
                           .center);
      solver.boundaryThermal[ib] =
          solution.Temperature[mesh->elementProjection[ie]] / distance1 +
          solution.Temperature[mesh->FourierMeshes
                                   .Elements[mesh->elementProjection[ie]]
                                   .faces[jface]
                                   .neighbor] /
              distance2;
      solver.boundaryThermal[ib] =
          solver.boundaryThermal[ib] / (1.0 / distance1 + 1.0 / distance2);
    }
  }
  MPI_Barrier(MPI_COMM_WORLD);
  if (Order == "1") {
    if (Method == "Synthetic") {
      if (Matrix_solver == "LU") {
        solver.solve_firstorder(Use_Backup, Num_Max_Iter, 0, error_temp_limit,
                                error_flux_limit);

      } else if (Matrix_solver == "BICGSTAB") {
        solver.solve_firstorder_Iterative(Use_Backup, Num_Max_Iter, 0,
      ...
      if (Matrix_solver == "LU") {
        solver.solve_DOM_firstorder(Use_Backup, Num_Max_Iter, 0,
                                    error_temp_limit, error_flux_limit);
      } else if (Matrix_solver == "BICGSTAB") {
        solver.solve_DOM_firstorder_Iterative(
            Use_Backup, Num_Max_Iter, 0, error_temp_limit, error_flux_limit);
      } else {
        cout << "no this matrix solver" << endl;
        exit(1);
      }
    ...
    if (Method == "Synthetic") {
      if (Matrix_solver == "LU") {
        solver.solve(Use_Backup, Num_Max_Iter, 0, error_temp_limit,
                     error_flux_limit);

      } else if (Matrix_solver == "BICGSTAB") {
        solver.solve_Iterative(Use_Backup, Num_Max_Iter, 0, error_temp_limit,
                               error_flux_limit);
      ...
      if (Matrix_solver == "LU") {
        solver.solve_DOM(Use_Backup, Num_Max_Iter, 0, error_temp_limit,
                         error_flux_limit);

      } else if (Matrix_solver == "BICGSTAB") {
        solver.solve_DOM_Iterative(Use_Backup, Num_Max_Iter, 0,
                                   error_temp_limit, error_flux_limit);
    ...
    if (Method == "Synthetic") {
      if (Matrix_solver == "LU") {
        solver.solve(Use_Backup, Num_Max_Iter, 1, error_temp_limit,
                     error_flux_limit);

      } else if (Matrix_solver == "BICGSTAB") {
        solver.solve_Iterative(Use_Backup, Num_Max_Iter, 1, error_temp_limit,
                               error_flux_limit);
      ...
      if (Matrix_solver == "LU") {
        solver.solve_DOM(Use_Backup, Num_Max_Iter, 1, error_temp_limit,
                         error_flux_limit);

      } else if (Matrix_solver == "BICGSTAB") {
        solver.solve_DOM_Iterative(Use_Backup, Num_Max_Iter, 1,
                                   error_temp_limit, error_flux_limit);
...
void SolutionAll::_get_solver(int i, DistributeMesh *mesh,
                              BTEBoundaryCondition *bcs, BTEBand *bands,
                              BTEAngle *angles, int num_proc, int world_rank,
                              int Use_Backup, int Num_Max_Iter,
                              int Use_Sythetic, int Use_Limiter,
                              double error_temp_limit,
                              double error_flux_limit) {

  if (solutions[i].solverType == "BTE") {
    StaticBTESynthetic solver(&mesh->mesh[i], bcs, bands, angles, num_proc,
                              world_rank);
    ...
    for (int j = 0; j < solutions[i].numBand * solutions[i].numDirection *
                            solutions[i].numBound * 2;
         ++j) {
...
void SolutionAll::_solve(DistributeMesh *mesh, BTEBoundaryCondition *bcs,
                         BTEBand *bands, BTEAngle *angles, int num_proc,
                         int world_rank, int Use_Backup, int Num_Max_Iter,
                         int Use_Sythetic, int Use_Limiter,
...
void SolutionAll::_Transient_BTE_Solver(
    DistributeMesh *mesh, BTEBoundaryCondition *bcs, BTEBand *bands,
    BTEAngle *angles, int num_proc, int world_rank, int Use_Backup,
    string Order, double error_temp_limit, double error_flux_limit,
    double deltaT, double totalT, int use_TDTR, double pulse_time,
    double repetition_frequency, double modulation_frequency, double xy_r) {
  Transient solver(&mesh->FourierMeshes, bcs, bands, angles, num_proc,
                   world_rank, deltaT, totalT, use_TDTR, pulse_time,
                   repetition_frequency, modulation_frequency, xy_r);
  for (int j = 0; j < solution.numCell; ++j) {
    solver.temperature[j] = solution.Temperature[j];
    solver.heatFluxXGlobal[j] = solution.heatFluxX[j];
    solver.heatFluxYGlobal[j] = solution.heatFluxY[j];
  ...
  if (Order == "2") {
    solver.solve(Use_Backup, error_temp_limit, error_flux_limit, deltaT, totalT,
                 use_TDTR, pulse_time, repetition_frequency,
                 modulation_frequency, xy_r);
  }
